module.exports = function(schema, option) {
   ...
   * 是否是第一次容器转化
   */
  let isFirstBuildContainer = true
   ...
   * DTD的url
   */
  const URL_DTD = 'https://filetest.ltwebstatic.com/filetest/2022/dynamic1029.dtd'
   ...
   * 解析事件属性
   */
  const parseEventAttrsFromProps = (props,key, attrs)=> {
    if (key !== 'onClick' && key !== 'onLongClick') {
      return
    }
    const targetFunction =  props[key]
    if (targetFunction && typeof targetFunction === 'function') {
      const targetFunctionString = targetFunction.toString().replace(/[\f\n\r\t\v]/gm,"")
      const start =  targetFunctionString.indexOf("{")
      const functionContent = targetFunctionString.substring(start+1, targetFunctionString.length-1).trim()
      attrs[key] =  "${" + functionContent + "}"
    }
  }
  ...
  const parseCommonAttrsFromStyle = (type, style) => {
    ...
    if (style.backgroundColor) {
      targetAttrs['background'] = translateRGBAColor2HexWithOpacity(
    ...
    if (style.borderBottomLeftRadius) {
      let borderLeftBottomRadius = getNumberUnitValue(
    ...
    if (style.borderBottomRightRadius) {
      let borderRightBottomRadius = getNumberUnitValue(
    ...
    if (
      ...
      if (
        ...
        style.borderTopRightRadius == style.borderBottomLeftRadius &&
        style.borderBottomLeftRadius == style.borderBottomRightRadius &&
        style.borderBottomRightRadius == style.borderTopLeftRadius
      ) {
    ...
    if (style.border) {
      if (style.border.indexOf(' ') == -1) {
        ...
        let borderColor = translateRGBAColor2HexWithOpacity(
          targetData[1].trim(),
          style.opacity
        )
        if (borderColor) {
  ...
  const parseContainerAttrsFromProps = (props) => {
    ...
    Object.keys(props).forEach((key) => {
      parseEventAttrsFromProps(props,key,attrs)
      if (key == 'style') {
    ...
    if (isFirstBuildContainer) {
      attrs['width'] = '${dynamicContainerWidth}'
      isFirstBuildContainer = false
    }
    return attrs
  }
  ...
  const parseTextAttrsFromProps = (props) => {
    ...
    Object.keys(props).forEach((key) => {
      ...
      if (key == 'style' || key == 'text') {
        ...
        if (style.color) {
          attrs['textColor'] = translateRGBAColor2HexWithOpacity(
            style.color,
            style.opacity
          )
        }
        ...
        if (
          ...
          attrs['verticalGravity'] =
            style.verticalAlign != 'middle' ? style.verticalAlign : 'center'
        }
        if (style.fontSize) {
    ...
    panelDisplay: [
      {
        ...
        panelValue: formatXML(
          `<?xml version="1.0" encoding="utf-8"?>\n<!DOCTYPE Flex PUBLIC "dynamic schema dtd 1.0 " "${URL_DTD}">\n${targetDSL}`
        ),
      },
      {
        panelName: `data.json`,
